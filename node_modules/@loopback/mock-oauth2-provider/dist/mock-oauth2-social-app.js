...
exports.stopApp = exports.startApp = void 0;
 ...
 */
async function createJwt(user, scopes, signingKey, clientId) {
...
async function verifyToken(token) {
    const unwrappedJwt = jsonwebtoken_1.default.decode(token, { json: true, complete: true });
    if (unwrappedJwt == null)
        throw new Error('invalid token');
    const tokenId = unwrappedJwt === null || unwrappedJwt === void 0 ? void 0 : unwrappedJwt.payload.jti;
    const registeredApp = registeredApps[unwrappedJwt === null || unwrappedJwt === void 0 ? void 0 : unwrappedJwt.payload.client_id];
    if (registeredApp) {
        const result = jsonwebtoken_1.default.verify(token, registeredApp[tokenId].signingKey);
        if (result) {
            return result;
        }
        else {
...
app.get('/login', function (req, response) {
    ...
    response.write('<input type="hidden" name=redirect_uri value="' +
        req.query.redirect_uri +
        '" />');
    response.write('<input type="hidden" name=client_id value="' +
        req.query.client_id +
        '" />');
...
app.get('/verify', async function (req, res) {
    var _a;
    try {
        const token = ((_a = req.query.access_token) !== null && _a !== void 0 ? _a : req.header('Authorization'));
        if (!token) {
...
//# sourceMappingURL=mock-oauth2-social-app.js.map